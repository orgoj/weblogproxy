				...
				- Contains sections for:
					...
					- `log_destinations`: A list of destinations where logs should be sent. Each destination defines:
						...
						- Type-specific settings:
						    - `file`: `path`, `format` ('json' or 'text'), `rotation` (optional: max_size in MB as a string value with minimum 1MB due to Lumberjack library limitations, `max_age` in duration format (e.g., "1d" for 1 day), `max_backups`, `compress`), `max_message_size` (optional, integer, max bytes per log line, default 4096; for JSON, replaces oversized messages with an error record). The 'json' format outputs Bunyan-compatible JSON Lines.
